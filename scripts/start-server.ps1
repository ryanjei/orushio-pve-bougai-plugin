...
try {
    ...
    Write-Host '現在のソースからpluginをbuildしています。'
    ...
    if (-not (Test-Path -LiteralPath $paperJar) -or (Get-Sha256 $paperJar) -ne $paperSha256) {
        Write-Host '公式Paper 1.21.11を取得しています。'
    ...
    Write-Host '最新plugin JARをPaperへ配置しています。'
    ...
    if ($PrepareOnly) { Write-Host '起動準備確認が完了しました。'; exit 0 }

    if (-not $NoBrowser) { $opener=Join-Path $PSScriptRoot 'open-admin.ps1';$arguments='-NoProfile -ExecutionPolicy Bypass -File "'+$opener+'" -Handoff "'+$handoff+'"';Start-Process powershell.exe -WindowStyle Hidden -ArgumentList $arguments | Out-Null }
