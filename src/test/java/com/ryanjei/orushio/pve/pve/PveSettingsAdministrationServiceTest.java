...
class PveSettingsAdministrationServiceTest {
    ...
    @Test void 未設定EnemyZoneもMapSetupから表示して順次保存できる(){settings.value=PveSettings.empty();var view=service.load("map-a");assertEquals(2,view.enemyZones().size());assertTrue(view.enemyZones().stream().noneMatch(PveSettingsAdministrationService.ZoneView::configured));var first=view.enemyZones().getFirst();var saved=service.update("map-a",update(view.revision(),first.enemyZoneId(),2,1,1,0,0,2,30));assertTrue(saved.enemyZones().getFirst().configured());assertFalse(saved.enemyZones().getLast().configured());assertEquals(1,settings.saves);}
    ...
    private MapProfile profile(){SetupMarker initial=new SetupMarker(UUID.randomUUID(),SetupMarkerType.INITIAL_RESPAWN,new BlockPoint(1,64,1,0,0),true);return new MapProfile(new MapProfileId("map-a"),"A",false,"template",Map.of(),Map.of("farmRegion",List.of(farm),"enemyZones",List.of(normal,finalZone),"finalRegion",List.of(finalRegion)),Instant.now(),List.of(),List.of(initial));}
