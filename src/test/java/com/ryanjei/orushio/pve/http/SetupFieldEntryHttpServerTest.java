...
class SetupFieldEntryHttpServerTest {
    ...
    @BeforeEach void start() throws Exception {
        ...
        server=new AdminHttpServer(InetAddress.getByName("127.0.0.1"),0,new DefaultGameApplicationService(new MemorySession()),administration,maps,new AuthService(),Map::of,new NoAudit(),false);
    ...
    @Test void entry変更対象を認証済みPUTで選択する() throws Exception {
        ...
        assertEquals(200,response.statusCode());assertEquals("shopPoints",maps.field);assertEquals(2,maps.index);
    }
    ...
    private static final class FakeMaps implements MapAdministrationService{
        ...
        public void selectFieldEntry(UUID administrator,String field,boolean area,int index){assertEquals(this.administrator,administrator);this.field=field;this.index=index;}
