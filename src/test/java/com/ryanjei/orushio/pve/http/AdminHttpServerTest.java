...
class AdminHttpServerTest {
    ...
    @Test void ホワイトリスト状態一覧追加削除APIが実状態を返し監査する() throws Exception {
        FakeAdministration administration = new FakeAdministration();
        CapturingAudit audit = new CapturingAudit();
        try (AuthenticatedServer test = new AuthenticatedServer(administration,audit)) {
            ...
            assertEquals("WHITELIST_UPDATED",audit.code);
    ...
    @Test void 既存OPの明示引継ぎは認証OriginCsrfを要求し監査する()throws Exception{FakeAdministration administration=new FakeAdministration();UUID playerId=UUID.randomUUID();administration.online.add(new OnlinePlayerView(playerId,"LegacyOp",true,false));CapturingAudit audit=new CapturingAudit();try(AuthenticatedServer test=new AuthenticatedServer(administration,audit)){var adopted=test.mutate("/api/v1/players/setup-administrator/ownership","POST","{\"uuid\":\""+playerId+"\"}");assertEquals(200,adopted.statusCode());assertTrue(adopted.body().contains("\"administratorRevocable\":true"));assertEquals("SETUP_ADMINISTRATOR_ADOPTED",audit.code);var unauth=HttpRequest.newBuilder(URI.create(test.baseUrl+"/api/v1/players/setup-administrator/ownership")).header("Origin",test.baseUrl).header("X-CSRF-Token",test.csrfValue).POST(HttpRequest.BodyPublishers.ofString("{\"uuid\":\""+playerId+"\"}")).build();assertEquals(401,client.send(unauth,HttpResponse.BodyHandlers.ofString()).statusCode());var badCsrf=HttpRequest.newBuilder(URI.create(test.baseUrl+"/api/v1/players/setup-administrator/ownership")).header("Cookie",test.cookieValue).header("Origin",test.baseUrl).header("X-CSRF-Token","bad").POST(HttpRequest.BodyPublishers.ofString("{\"uuid\":\""+playerId+"\"}")).build();assertEquals(403,client.send(badCsrf,HttpResponse.BodyHandlers.ofString()).statusCode());}}
    ...
    private static final class CapturingAudit implements AuditSink {String trace,code;public void record(String t,String c,String value,String op){trace=t;code=value;}public boolean healthy(){return true;}}
