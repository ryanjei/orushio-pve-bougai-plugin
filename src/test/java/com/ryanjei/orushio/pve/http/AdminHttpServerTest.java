...
class AdminHttpServerTest {
    ...
    @Test void セットアップ管理者付与APIは認証OriginCsrfを要求しオンラインUUIDだけを受け付ける() throws Exception {
        ...
        try (AuthenticatedServer test = authenticated(administration)) {
            var success=test.mutate("/api/v1/players/setup-administrator","PUT","{\"uuid\":\""+playerId+"\"}");assertEquals(200,success.statusCode());assertTrue(success.body().contains("\"setupAdministrator\":true"));
