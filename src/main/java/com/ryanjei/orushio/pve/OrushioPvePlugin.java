...
public final class OrushioPvePlugin extends JavaPlugin {
    ...
    @Override public void onEnable(){Path data=getDataFolder().toPath();try{Files.createDirectories(data);StartupState startup=StartupState.load(data);RuntimeConfiguration config=startup.configuration();AuditLog audit=new AuditLog(data.resolve("logs"));DefaultGameApplicationService games=createGames(data,startup);gameThread=new PaperGameThreadExecutor(this);ensureSecrets(data,config);final boolean[] bound={false};http=new AdminHttpServer(InetAddress.getByName("127.0.0.1"),config.port(),games,new PaperPlayerQuery(gameThread),new AuthService(),()->diagnostics(startup,audit,bound[0],data,games),audit,startup.diagnosticMode());http.start();bound[0]=true;audit.record(config.traceId(),"SYSTEM",startup.diagnosticMode()?"DIAGNOSTIC_START":"NORMAL_START","PLUGIN_ENABLE");getLogger().info(startup.diagnosticMode()?"Orushio PVEを診断モードで起動しました。":"Orushio PVE基盤を起動しました。管理画面: http://127.0.0.1:"+config.port()+"/");}catch(Exception e){getLogger().severe("管理HTTPを安全に起動できないためプラグインを停止します。追跡ID: "+UUID.randomUUID());getServer().getPluginManager().disablePlugin(this);}}
