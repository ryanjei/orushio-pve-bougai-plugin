...
public final class DefaultGameApplicationService implements GameApplicationService {
    ...
    public synchronized void validateMapSetup(String expectedSessionId){if(session.state()!=GameState.MAP_SETUP)throw new DomainException("GAME_STATE_CONFLICT","現在はマップセットアップ中ではありません。");if(!session.sessionId().toString().equals(expectedSessionId))throw new DomainException("SESSION_MISMATCH","画面のセッションが現在のセッションと一致しません。");}
    public synchronized OperationResult closeMapSetup(String expectedSessionId){validateMapSetup(expectedSessionId);session.transitionedTo(GameState.IDLE);GameSession next=GameSession.newIdle();repository.save(next);session=next;return result();}
