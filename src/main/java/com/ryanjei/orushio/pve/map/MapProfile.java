...
public record MapProfile(MapProfileId mapId, String displayName, boolean enabled, String templateDirectory,
    ...
    public MapProfile {
        ...
        Set<UUID> setupMarkerIds=new HashSet<>();for(SetupMarker marker:setupMarkers)if(!setupMarkerIds.add(marker.markerId())||markerIds.contains(marker.markerId()))throw new IllegalArgumentException("markerIdが重複しています。");if(enabled&&!missing(points,areas).isEmpty())throw new IllegalArgumentException("必須設定が不足しているため有効化できません。");
    }
