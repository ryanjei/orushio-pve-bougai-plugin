# OPBP（Orushio PvE Bougai Plugin）
...
1. Repository最上位の `START_SERVER.bat` をダブルクリックします。
   ダブルクリック直後に「OPBP サーバーを起動しています...」と表示されます。
...
- OPBPから付与した管理者権限は「管理者を解除」で安全に解除できます。もともとサーバー管理者だったプレイヤーのOPは解除しません。
- 以前の版で付与済みのOPが「既存OP / OPBPからは解除不可」と表示された場合は、対象本人がオンラインの状態で「OPBP管理として引き継ぐ」を押し、確認画面で承認してください。OP状態はその場では変わらず、その後に管理画面から安全に解除できるようになります。
...
4. 設定状況には日本語名、必須／任意、設定済み／未設定、必要件数と編集中Draftの座標が表示されます。地点は「テレポート」、範囲は「中央へテレポート」で現在のセットアップ管理者を移動できます。
5. 誤登録した地点・範囲は、座標横の「削除」から確認後にDraftだけを削除できます。必須数を下回ると即座に未完了表示になります。
6. 「保存終了」はDraftを保存済みMapProfileへ反映し、「破棄終了」は今回のDraft変更を捨てます。どちらも通常ワールドへ戻り、専用ツールは回収されます。

主な設定項目は、ファーム範囲・開始地点、攻略入口、通常コア候補3地点、最終コア、最終ゲート範囲、最終エリア範囲、最終入口、Enemy Zone範囲です。チェックポイント、資源ゾーン、ショップ地点はマップに応じて登録します。

## 終了方法

`START_SERVER.bat`で開いたウィンドウで`Y`キーを押してください。起動補助がlocalhost限定のOPBP安全停止機能へ1回限りの認証付き要求を送り、プラグインがPaper正式APIで停止します。ワールド保存と終了を待ち、「OPBPサーバーを安全に停止しました。」と表示されるまでウィンドウを閉じたりPCを終了したりしないでください。要求に失敗した場合も強制終了せず、サーバーが稼働中であることを表示します。`Ctrl+C`、Paperコンソールの`stop`、コマンド入力は正式な終了操作ではありません。
