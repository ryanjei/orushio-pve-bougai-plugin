...
Minecraft Java版のPaperサーバーで、視聴者参加型の妨害＋PvE企画を運営するためのプラグインです。現在のPhase 4.1では、日本語Web管理画面、ホワイトリスト管理、安全なマップZIP登録・セットアップに加え、ゲーム参加者とゲーム開始・中止・制限時間を管理できます。ワールド生成を伴うPvE本体と妨害は後続Phaseです。
...
4. 「内容を確認して準備開始」で確認画面を承認します。この時点は「ゲーム準備中」であり、まだゲーム進行中ではありません。
5. 準備が整ったら「準備完了としてゲーム進行開始」を押します。制限時間はサーバー時刻を基準に計測されます。
6. 必要な場合は「ゲームを中止」で安全な中止処理へ進めます。

参加者が切断しても開始時人数や参加者登録は減りません。切断中の後処理は清掃待ちとして保存されます。準備中・進行中などの状態でサーバーが予期せず停止した場合、再起動後は新しいゲームを開始せず診断モードになります。Phase 4.1は管理基盤までで、ゲーム用一時ワールドへの転送やPvE要素の生成はまだ行いません。
