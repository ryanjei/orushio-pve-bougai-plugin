...
Minecraft Java版のPaperサーバーで、視聴者参加型の妨害＋PvE企画を運営するためのプラグインです。現在のPhase 4.2では、日本語Web管理画面、ホワイトリスト管理、安全なマップZIP登録・セットアップ、ゲーム参加管理に加え、1ゲーム専用ワールドの作成・参加者転送・安全な回収を行えます。敵・コア・資源・ショップと妨害は後続Phaseです。
...
6. Enemy Zone、ファーム、資源ゾーン、最終エリアには、Area別の「地点を追加」からスポーン候補地点を登録できます。地点一覧はAreaごとに折りたたまれ、座標変更・有効／無効・テレポート・削除を扱えます。Minecraftでクリックした結果は画面全体を暗転させず自動反映されます。
7. 「保存終了」はDraftを保存済みMapProfileへ反映し、「破棄終了」は今回のDraft変更を捨てます。スポーン地点も保存するまではDraftだけが変わります。どちらも通常ワールドへ戻り、専用ツールは回収されます。

主な設定項目は、ファーム範囲・開始地点、攻略入口、通常コア候補3地点、最終コア、最終ゲート範囲、最終エリア範囲、最終入口、Enemy Zone範囲です。チェックポイント、資源ゾーン、ショップ地点はマップに応じて登録します。

## ゲーム参加者と開始

1. Minecraftへ参加者が接続した状態で、管理画面の「ゲーム操作」を開きます。
2. 使用する有効なセットアップ済みマップを選び、オンライン一覧から1～4人をゲーム参加者へ追加します。ゲーム開始後の追加・解除はできません。
3. 必要なら制限時間、攻略通常コア数、敵人数倍率を入力して「マップ別設定を保存」を押します。空欄は既定値（60分、2個、1.0倍）です。
4. 「内容を確認して準備開始」で確認画面を承認します。この時点は「ゲーム準備中」です。
5. 「準備完了としてゲーム進行開始」を押すと、選択マップの原本から1ゲーム専用ワールドを作り、準備に成功した後でオンライン参加者を攻略入口へ転送します。オフライン参加者の登録は維持され、進行中の再接続時に攻略入口へ復帰します。
6. 必要な場合は「ゲームを中止」で安全な中止処理へ進めます。

参加者が切断しても開始時人数や参加者登録は減りません。ゲーム終了時はオンラインPlayerを通常worldへ戻してから専用ワールドを回収し、オフライン参加者は次回接続時にロビー復帰を完了します。準備中・進行中などの状態でサーバーが予期せず停止した場合は復旧清掃状態となり、新しいゲームを開始しません。Phase 4.2ではPvEの敵・コア・資源・ショップはまだ生成しません。
...
- [Phase 4.1 handoff](docs/handoffs/PHASE_4_1_GAME_LIFECYCLE_HANDOFF.md)
- [Phase 4.2 handoff](docs/handoffs/PHASE_4_2_MAP_RUNTIME_HANDOFF.md)
