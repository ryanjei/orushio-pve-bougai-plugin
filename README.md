...
`START_SERVER.bat`で開いたウィンドウで`Y`キーを押してください。起動補助がPaperへ安全な停止要求を送り、ワールド保存と終了を待ちます。「OPBPサーバーを安全に停止しました。」と表示されるまでウィンドウを閉じたりPCを終了したりしないでください。`Ctrl+C`、Paperコンソールの`stop`、コマンド入力は正式な終了操作ではありません。
