...
- 管理画面は編集中Draftの地点・範囲座標を日本語項目名ごとに表示する。個別削除はDraftだけを変更し、登録座標への移動は現在のセットアップ管理者と対象setup worldへ限定する。
...
- `DELETE /api/v1/maps/setup/entry`
- `POST /api/v1/maps/setup/teleport`
...
7. 管理画面のオンラインプレイヤー一覧で自分の「セットアップ管理者にする」を押し、「管理者利用可能」へ変わることを確認する。旧版で付与済みのため「既存OP / OPBPからは解除不可」と表示される場合は、「OPBP管理として引き継ぐ」を押して確認画面で承認する。
...
13. 管理画面へ編集中Draftの地点・範囲座標、件数、不足項目が反映されることを確認し、地点の「テレポート」と範囲の「中央へテレポート」を確認する。
14. 保存終了し、ロビーワールドへ帰還することを確認する。
15. `orushio_setup_*`一時ワールドがアンロード・削除されたことを確認する。
16. 登録原本のファイル内容・更新時刻が変化していないことを確認する。
17. 同じマップで再度セットアップを開始し、保存済みの`farmSpawn`座標Aを確認してから別の座標Bへ変更し、管理画面がDraftのBを表示することを確認する。必要に応じて座標削除と不足表示も確認する。
18. 破棄終了する。
19. 再度セットアップを開始し、`farmSpawn`が保存済み座標Aへ戻っていることを確認する。
20. `START_SERVER.bat`のウィンドウで`Y`キーを押し、「OPBPサーバーを安全に停止しました。」まで確認する。
...
6. setupを開始し、保存済み座標Aを管理画面で確認してから1地点をBへ再設定し、Minecraftの日本語項目名メッセージと管理画面のDraft座標Bを確認する。テレポートと個別削除も確認する。
7. 保存終了し、通常worldへの帰還と設定ツール回収を確認する。
8. 再setupで同じ値を変更し、破棄終了後の再setupで保存済み座標Aが維持されることを確認する。
9. `Y`キーで安全停止し、ランチャーからlocalhost限定のOPBP停止APIへ要求され、PaperコンソールにUnknown commandが出ないこと、world保存、plugin disable、終了コード0、「OPBPサーバーを安全に停止しました。」の表示を確認する。
