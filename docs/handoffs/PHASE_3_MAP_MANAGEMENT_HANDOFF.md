...
6. Minecraft Java Edition 1.21.11から`127.0.0.1:25565`へ、OPの管理者プレイヤーで接続する。
7. `level.dat`を含む小規模なJava版テストワールドZIPを登録する。
8. オンラインかつOP・`orushio.pve.admin`権限を持つ管理者を選び、マップセットアップを開始する。
9. 一時ワールドへ転送され、専用のマップ設定ツールを受け取ることを確認する。
10. 管理画面で`farmSpawn`を選び、ツールで単一点を登録する。
11. 管理画面で`farmRegion`を選び、左クリックで1点目、右クリックで2点目を登録する。
12. 管理画面のセットアップ状態・不足項目へ反映されることを確認する。
13. 保存終了し、ロビーワールドへ帰還することを確認する。
14. `orushio_setup_*`一時ワールドがアンロード・削除されたことを確認する。
15. 登録原本のファイル内容・更新時刻が変化していないことを確認する。
16. 同じマップで再度セットアップを開始し、`farmSpawn`または`farmRegion`を別の値へ変更する。
17. 破棄終了する。
18. 再読込み後のMapProfileが手順13で保存した内容から変化していないことを確認する。
19. `START_SERVER.bat`のウィンドウで`Ctrl+C`を1回押し、Paperが安全に停止することを確認する。
