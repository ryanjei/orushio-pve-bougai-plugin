...
| 通常認証 | ランダムなインストール秘密＋ブラウザセッション。Phase 5ランチャーがワンタイムbootstrap URLを開く |
| 外部公開 | v1では禁止 |
...
- Phase 5のローカルランチャーは、短時間・1回限りのbootstrap tokenを安全なローカル連携で取得してブラウザを開く。
